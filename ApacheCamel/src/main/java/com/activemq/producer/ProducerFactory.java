...
public class ProducerFactory {
  ...
  public void createQueueProducer(String queue) throws JMSException {
    session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
    Destination destination = session.createQueue(queue);
    messageProducer = session.createProducer(destination);
  }

  public void createTopicProducer(String queue) throws JMSException {
    session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
    Destination destination = session.createTopic(queue);
    messageProducer = session.createProducer(destination);
  }

  public void sendMessages(String groupId, int i) throws JMSException {
  ...
  public void sendMessagesWithPriority(String groupId, int i) throws JMSException {
    String msg = "Message-" + i;
    TextMessage message = session.createTextMessage(msg);
    message.setStringProperty("JMSXGroupID", groupId);
    message.setIntProperty("JMSXGroupSeq", i);
